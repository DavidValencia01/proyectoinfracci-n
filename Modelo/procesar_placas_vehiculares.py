...
def procesar_video(model, video_path, output_path=None, conf_threshold=0.40):
    """
    Procesa un video para detectar placas vehiculares y guarda el resultado
    """
    # Abrir el video
    ...
    if output_path is None:
        ...
        output_path = f"videos_salida/placas/{video_name}_procesado.mp4"
    ...
    while True:
        ...
        # Color rosado para las detecciones (BGR format)
        pink_color = (203, 192, 255)  # BGR: (203, 192, 255) = Rosado claro
        ...
        if results.boxes is not None:
            for box in results.boxes:
                ...
                cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), pink_color, 2)
        ...
        cv2.putText(annotated_frame, f"Placas detectadas: {num_detections}", 
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, pink_color, 2)
...
def procesar_frame(model, frame, conf_threshold=0.40):
    annotated_frame = frame.copy()
    results = model.predict(frame, conf=conf_threshold)[0]
    boxes_placa = []
    if results.boxes is not None:
        for box in results.boxes:
            x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
            x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
            boxes_placa.append([x1, y1, x2, y2])
            pink_color = (203, 192, 255)
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), pink_color, 2)
            label = f"Placa {float(box.conf[0]):.2f}"
            (text_width, text_height), baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.rectangle(annotated_frame, (x1, y1 - text_height - baseline - 5), (x1 + text_width, y1), pink_color, -1)
            cv2.putText(annotated_frame, label, (x1, y1 - baseline - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    return annotated_frame, boxes_placa
...
def main():
    ...
    if len(sys.argv) >= 3:
        ...
        # (asume que tienes una función procesar_video similar)
        model_path = "runs/detect/license_plate_detector_v1/weights/best.pt"
    ...
    output_dir = Path("videos_salida/placas")
