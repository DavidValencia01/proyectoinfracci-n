...
def test_on_image(model, image_path):
    ...
    output_path = Path("runs/detect/test_results_plate_characters")
...
def main():
    ...
    model_path = "runs/detect/plate_characters_detector_v1/weights/best.pt"
    ...
    test_dir = Path("PlateCharacters.v1i.yolov8/test/images")
    ...
    total_detections = 0
    detections_by_class = {name: 0 for name in model.names.values()}
    confidence_by_class = {name: [] for name in model.names.values()}
    ...
    print(f"Número de clases detectadas: {len([c for c in detections_by_class.values() if c > 0])}")
    
    # Mostrar estadísticas por clase (solo las que tienen detecciones)
    ...
    # Mostrar clases con más detecciones
    print("\n=== Top 10 Clases con Más Detecciones ===")
    sorted_classes = sorted(detections_by_class.items(), key=lambda x: x[1], reverse=True)
    for i, (class_name, detections) in enumerate(sorted_classes[:10]):
        if detections > 0:
            confidences = confidence_by_class[class_name]
            avg_conf = np.mean(confidences) if confidences else 0
            print(f"{i+1}. {class_name}: {detections} detecciones (conf. prom: {avg_conf:.2f})")
